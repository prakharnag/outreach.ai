...
function determineCompanySize(researchData: any): string {
  try {
...
function extractIndustry(researchData: any): string {
  try {
    const data = typeof researchData === 'string' ? JSON.parse(researchData) : researchData;
...
export async function GET(request: NextRequest) {
  try {
    ...
    // Fetch contact results for the user
    const { data: contactResults, error } = await supabase
    ...
    contactResults.forEach((result: any) => {
      // Industry analysis
    ...
    const companySizeData: CompanySizeData[] = Array.from(sizeMap.entries())
      .map(([size, count]) => ({
        size,
        count,
    ...
    const researchTrends: ResearchTrend[] = last7Days.map(date => {
